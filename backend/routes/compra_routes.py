...
@router.get("")
def get_compras(connection=Depends(get_db)):
...
def get_compras_by_proveedor_id(
    ...
    return compra_service.get_compras_by_proveedor_id(proveedor_id)


@router.get("/{compra_id}")
def get_compra(compra_id: int, connection=Depends(get_db)):
    compra_service = CompraService(connection)

    return compra_service.get_compra_by_id(compra_id)
...
def create_compra(
    ...
    return compra_service.create_compra(compra_data)
...
def delete_compra(compra_id: int, connection=Depends(get_db)):
    compra_service = CompraService(connection)

    return compra_service.delete_compra(compra_id)